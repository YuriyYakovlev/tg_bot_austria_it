...
async function cacheUserMessage(userId, chatId, messageId, text) {
    try {
      await db.query(`INSERT INTO cached_messages (userId, chatId, messageId, messageText) VALUES (?, ?, ?, ?)`, [userId, chatId, messageId, text]);
  ...
  async function retrieveCachedMessages(userId) {
    try {
      const [rows] = await db.query(`SELECT messageId, messageText FROM cached_messages WHERE userId = ?`, [userId]);
      console.log(`Retrieved cached messages for user ${userId}`);
      if (rows.length === 0) {
        console.log(`No cached messages found for user ${userId}`);
      } else {
        console.log(`Retrieved ${rows.length} cached messages for user ${userId}`);
      }
      return rows;
    } catch (error) {
      console.error(`Error retrieving cached messages for user ${userId}:`, error);
      return [];
    }
  }
  ...
  async function deleteCachedMessage(messageId) {
    try {
      await db.query(`DELETE FROM cached_messages WHERE messageId = ?`, [messageId]);
